              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in itemsWithIndex" :key="item._id">
                    <td>{{item.index+1}}</td>
                    <td>{{ item.img }}</td>
                    <td v-if="item.question_id">{{ item.question_id.question }}</td>
                    <td v-else>-</td>
                    <td v-if="item.question_id"> 
                      <!-- {{item.question_id}} -->
                         <ul v-if="item.question_id.ans_type == 'Choice'">
                              <li v-for="(item,index) in item.question_id.ans" :key="index">

                              <div v-if="item.correct">
                                  <font color="green">({{ index +1 }})  {{ item.ans}} </font>  
                              </div>

                              <div v-else>
                                  <font>({{ index +1 }})  {{ item.ans}} </font>  
                              </div>
                    ...
                    <!-- <p v-if="item.question_id" > -->
                      <div v-if="item.question_id" >
